...
export function safeCommand(command: string): string { return commands.has(command) ? command : "unknown"; }
export function safeError(error: unknown): string {
  const message = (error instanceof Error ? error.message : typeof error === "string" ? error : "").slice(0, 4096).toLowerCase();
  if (message.includes("model and effort must be identifiers")) return "invalid_launch_identifier";
  if (message.includes("timeout") || message.includes("timed out")) return "timeout";
  if (message.includes("permission") || message.includes("denied") || message.includes("unauthorized")) return "permission_denied";
  if (message.includes("not found") || message.includes("no such file")) return "not_found";
...
export function diagnosticOperation(sessionId: unknown): string | undefined {
  if (typeof sessionId!=="string") return undefined;
  const value=operations.get(sessionId);
  if (value && value.until>Date.now()) return value.id;
  operations.delete(sessionId);return undefined;
}
